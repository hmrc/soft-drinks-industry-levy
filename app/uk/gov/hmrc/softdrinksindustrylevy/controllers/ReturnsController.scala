...
import uk.gov.hmrc.softdrinksindustrylevy.connectors.SubscriptionConnector
...
class ReturnsController @Inject() (
    ...
    Action.async(parse.json) { implicit request =>
      authorised(AuthProviders(GovernmentGateway)).retrieve(credentials) { creds =>
        withJsonBody[SdilReturn] { sdilReturn =>
          ...
          (for {
            subscription <- subscriptionConnector.retrieveSubscriptionDetails("utr", utr)
            ref = subscription.get.sdilRef.get
            _ <- subscriptionConnector.submitReturn(ref, returnsReq)
  ...
  def pending(utr: String): Action[AnyContent] =
    Action.async { implicit request =>
      subscriptionConnector.retrieveSubscriptionDetails("utr", utr).flatMap { subscription =>
        subscription match {
          case Some(value) =>
            val all = {
