...
object JsonSchemaChecker {
  ...
  def checkAgainstSchema[A](subscription: A)(implicit format: Format[A]): Unit = {
    val validator = JsonSchemaFactory.byDefault.getValidator
    val json = JsonLoader.fromString(Json.prettyPrint(Json.toJson(subscription)))
    val processingReport: ProcessingReport = validator.validate(subscriptionSchema, json)
