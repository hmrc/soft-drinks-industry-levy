...
class BalanceControllerSpec extends FakeApplicationSpec with MockitoSugar with BeforeAndAfterEach with ScalaFutures {
  ...
  "should convert" in {
    ...
    // convertwitoutassessments

    val x = BalanceController.convertWithoutAssessment(obj)

    //deduplicate payments sorted by the date
    val y = BalanceController.deduplicatePayments(x).sortBy(_.date.toString)

    //respond with json array
      // standard jsArray
//    Json.toJson(y) must be(4)

      // jsArray with obj to json
    JsArray(y.map {Json.toJson(_)}) must be(4)
  }
